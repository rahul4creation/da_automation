...
const createdUsersKey = "da-review-ai-ui-created-users";
...
const phaseGuidance: Record<string, { userAction: string; uploads: string; output: string; gateFocus: string }> = {
  ...
  "05-ai-review-validation": {
    userAction: "Review the generated Excel/PDF finding files, quality checks, reconciliation evidence, and open issues.",
    uploads: "Generated review JSON, Markdown summaries, design matrix, data validation matrix, selected checklist, Excel report, or PDF report.",
...
export default function App() {
  ...
  function handleLogin(username: string, password: string) {
    const createdUser = findCreatedUser(username);
    const validCreatedUser = createdUser && createdUser.password === password;
    if ((username === validLogin.username && password === validLogin.password) || validCreatedUser) {
      sessionStorage.setItem(authKey, "true");
      sessionStorage.setItem(authUsernameKey, username);
  ...
  function handleCreateUser(input: CreateUserInput): CreateUserResult {
    const adminUsername = input.adminUsername.trim();
    const adminPassword = input.adminPassword;
    const username = input.username.trim();
    const password = input.password;

    if (adminUsername !== validLogin.username || adminPassword !== validLogin.password) {
      return { ok: false, message: "Only Rahul_Raj can create users. Enter Rahul_Raj admin credentials." };
    }
    if (!username || !password) {
      return { ok: false, message: "New username and password are required." };
    }
    if (username === validLogin.username) {
      return { ok: false, message: "Rahul_Raj already exists as the admin user." };
    }
    const users = readCreatedUsers();
    ...
    const nextUsers = [
      ...users,
      {
        username,
        password,
        createdBy: validLogin.username,
        createdAt: new Date().toISOString()
      }
    ];
    writeCreatedUsers(nextUsers);
    localStorage.setItem(lastUsernameKey, username);
    setLastUsername(username);
    return { ok: true, message: `User ${username} created successfully.`, username };
  }
            ...
            <section className="phase-rail">
              ...
              <div className="phase-list">
                {project.phases.map((phase) => {
                  const draftTitle = phaseRenameDrafts[phase.id] ?? phase.title;
                  const changed = draftTitle.trim() !== phase.title && draftTitle.trim().length > 0;
                  return (
                    <div
                      key={phase.id}
                    ...
                    >
                      ...
                      <span className="phase-title-edit">
                        <input
                          aria-label={`Rename phase ${phase.number}`}
                          onChange={(event) => setPhaseRenameDrafts((current) => ({ ...current, [phase.id]: event.target.value }))}
                          onClick={(event) => event.stopPropagation()}
                          onFocus={() => setActivePhaseId(phase.id)}
                          onKeyDown={(event) => {
                            if (event.key === "Enter") {
                              event.preventDefault();
                              void savePhaseRename(phase);
                            }
                          }}
                          value={draftTitle}
                        />
            ...
            <section className="phase-workbench">
              ...
              {activePhase.id === "05-ai-review-validation" ? (
                <>
                  ...
                  />
                  <div className="phase-artifact-workflow">
                    <QuestionnairePanel
                      phase={activePhase}
...
function LoginScreen({
  ...
  onCreateUser: (input: CreateUserInput) => CreateUserResult;
  ...
  function submitCreateUser(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const result = onCreateUser(createUserForm);
...
function readCreatedUsers() {
  try {
    const parsed = JSON.parse(localStorage.getItem(createdUsersKey) || "[]");
...
function writeCreatedUsers(users: CreatedUser[]) {
  localStorage.setItem(createdUsersKey, JSON.stringify(users));
}

function findCreatedUser(username: string) {
  const requested = username.trim().toLowerCase();
  return readCreatedUsers().find((user) => user.username.toLowerCase() === requested) || null;
}
...
function ReviewAgentInfo() {
  const features = [
    ...
    "Selected Excel and PDF report data validation",
    "Cross-PDF date range and value comparison",
  ...
  return (
    <section className="review-agent-info">
      <div className="review-agent-info-main">
        ...
        <strong>Excel PDF Data Review Agent</strong>
      </div>
