...
const APP_URL = process.env.SOP_APP_URL || "http://127.0.0.3:5190/";
...
async function captureScreenshots() {
  ...
  try {
    ...
    const projectDropdown = page.locator(".project-picker-panel select").first();
    if ((await projectDropdown.count()) > 0) {
      const options = await projectDropdown.locator("option").all();
      let selectedValue = "";
      ...
      if (!selectedValue && options.length > 1) selectedValue = (await options[1].getAttribute("value")) || "";
      if (selectedValue) await projectDropdown.selectOption(selectedValue);
    }
    await waitStable(page);
...
API_PORT=8802</pre>
  ...
  <ul>
    ...
    <li>If selected reports are mixed or not recognized as one family, the output is categorized under <strong>Miscellaneous</strong>.</li>
  </ul>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr><td><strong>Miscellaneous</strong></td><td>Selected files are mixed, unknown, or not one recognized family.</td><td><code>D:\\AIReview\\project\\UPCL REPORT\\Miscellaneous\\20-07-2026 16-04-50\\</code></td></tr>
    </tbody>
  </table>
  ...
  <table class="checklist">
    ...
    <tbody>
      ...
      <tr><td>4</td><td>Check report-family category folder.</td><td>Analog, SAIFI/SAIDI, or Miscellaneous folder is created correctly based on report header names.</td></tr>
